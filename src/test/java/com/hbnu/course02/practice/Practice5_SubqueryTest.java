 ...
 *
 * 【练习要求】
 * 实现下面两个方法，分别用 inSql 和 exists 写子查询。
 *
 * 安全铁律：子查询字符串只写常量，不要拼接任何变量（SQL 注入风险）。
 *
 * 完成后删除 @Disabled 注解。
 */
@SpringBootTest
class Practice5_SubqueryTest {
    ...
    /**
     * 任务①：用 inSql 查询"所属部门 company_id = 1 的员工"。
     *
     * 提示：用 .inSql(Employee::getDepartmentId, "SELECT id FROM dept WHERE company_id = 1")
     */
    ...
    /**
     * 任务②：用 exists 查询"所属部门 active = 1 的员工"。
     *
     * 提示：用 .exists("SELECT 1 FROM dept WHERE dept.id = employee.department_id AND dept.active = 1")
     */
